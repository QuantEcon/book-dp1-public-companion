import numpy as np
from finite_opt_saving_0 import U
...
def get_greedy(v, model):
    ...
    σ = np.empty((w_grid.shape[0], y_grid.shape[0]))
...
def get_value(σ, model):
    ...
    r_σ = np.zeros(n)
    for m in prange(n):
        i, j = single_to_multi(m, wn)
        w, y, w_1 = w_grid[i], y_grid[j], w_grid[σ[i, j]]
        r_σ[m] = U(w + y - w_1/R, γ)
        for m_1 in prange(n):
            i_1, j_1 = single_to_multi(m_1, wn)
            if i_1 == σ[i, j]:
                P_σ[m, m_1] = Q[j, j_1]
